...
def distance_calc(ry, ss, h: float = 0,  shift: float = 0, debug: bool = False):
    ...
    for edge in range(edges):  # The platform has 6 corners
        if counter % 2 == 0:
            ...
            print(f"counter par = {counter} - applied gamma factor: {counter / 2 - 1}, applied gamma = {applied_gamma}")
        elif counter % 2 != 0:
            # if debug:
            applied_gamma = gamma * ((counter-1)/2)
            print(f"counter odd = {counter} - applied gamma factor: {(counter - 1) / 2}, applied gamma = {applied_gamma}")
